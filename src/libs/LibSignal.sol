...
library LibSignal {
    ...
    function signaled(address account, bytes32 value) internal view returns (bool) {
        return deriveSlot(account, value).getBytes32Slot().value != 0;
    }
    ...
    function signal(address account, bytes32 value) internal returns (bytes32) {
        bytes32 slot = deriveSlot(account, value);
    ...
    function deriveSlot(address account, bytes32 value) internal pure returns (bytes32) {
    ...
    function verifySignal(
        ...
        (valid,) = LibTrieProof.verifyStorage(
            address(this), deriveSlot(sender, value), value, root, accountProof, storageProof
        );
    }
    ...
    /// @return valid Boolean indicating whether the signal was successfully verified
    function verifySignal(bytes32 root, address sender, bytes32 value, bytes[] memory stateProof)
        internal
        pure
        returns (bool valid)
    {
