...
abstract contract AddressResolver is IAddressResolver, Initializable {
    ...
    modifier onlyFromNamedEither(bytes32 _name1, bytes32 _name2) {
        require(msg.sender == resolve(_name1, true) || msg.sender == resolve(_name2, true), RESOLVER_DENIED());
        _;
    }
    ...
    /// @inheritdoc IAddressResolver
    function resolve(uint64 _chainId, bytes32 _name, bool _allowZeroAddress) public view virtual returns (address) {
    ...
    /// chain.
    function _resolve(uint64 _chainId, bytes32 _name, bool _allowZeroAddress) internal view returns (address addr_) {
