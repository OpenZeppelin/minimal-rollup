...
contract NativeTokenBridge is INativeTokenBridge {
    ...
    function verifyClaim(
        ...
        id = transferId(sourceChainId, blockNumber, from, to, value);
        return (LibTrieProof.verifyState(id.deriveSlot(), id, root, proof), id);
    }

    function transfer(address to) external payable virtual {
        address from = msg.sender;
        uint64 blockNumber = block.number.toUint64();
        transferId(block.chainid.toUint64(), blockNumber, from, to, msg.value).signal();
        emit Transfer(blockNumber, from, to, msg.value);
    }
