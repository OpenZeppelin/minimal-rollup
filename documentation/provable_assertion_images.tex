...
\newcommand{\injectAnchorToBlock}[1]{
    ...
    \node[draw, lTwoTx, fill=\anchorColor, minimum width=0.1cm, anchor=west] at (l2Tx-#1-0.west) {};
    \draw[->, \anchorColor] (\anchorBlock.south) to[bend right] (l2Tx-#1-0.west);
}
...
\begin{tikzpicture}[framed]
    ...
    \node[draw, lTwoTx, fill=green] at (l2Tx-1-3) {};
    \node[draw, lTwoTx, fill=olive] at (l2Tx-3-1) {};
    \node[draw, lTwoTx, fill=olive, minimum width=0.1cm, anchor=west] (assertion) at (l2Tx-1-2.west) {};
    \draw[->, olive] (l2Tx-3-1.west) to[bend right] (assertion);
\end{tikzpicture}
\fi
