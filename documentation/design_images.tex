...
% Usage: blobs{X}{sizeA}{sizeB} where 
%   X is the number of blobs in a group
%   sizeA and sizeB are the number of blobs the rollup segments occupy (todo: compute sizeB = X - sizeA)
\renewcommand{\blobs}[3]{
    \switchToA
    \node (nextSegment) at (nextBlob.east) [segment, minimum width=#2 * 0.5cm] {};
    \switchToB
    \node (nextSegment) at (nextSegment.east) [segment, minimum width=#3 * 0.5cm] {};
